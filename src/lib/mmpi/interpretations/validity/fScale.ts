// src/lib/mmpi/interpretations/scales/fScale.ts
...
export const fScaleInterpretation = {
  ...
  getInterpretationByTScore(tScore: number) {
    if (tScore >= 80) return this.tScoreInterpretations[0];
    if (tScore >= 70) return this.tScoreInterpretations[1];
    if (tScore >= 55) return this.tScoreInterpretations[2];
    if (tScore >= 44) return this.tScoreInterpretations[3];
    if (tScore < 44) return this.tScoreInterpretations[4]; // Note: Text says <45, but range above is 44-54, so <44 is more logical.
    return undefined;
  }
};
